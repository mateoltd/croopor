...
export function Onboarding(): JSX.Element | null {
    ...
    const microsoftSignInAvailable =
        hasNativeDesktopRuntime() || authStatus?.login_available !== false;
    const microsoftLogin = useMicrosoftSignIn({
        ...
        onAuthenticated: async (result) => {
            ...
            try {
                ...
                console.warn(
                    "Could not refresh Microsoft sign-in status during onboarding.",
                    err,
                );
            ...
            const onlineReady = refreshedStatus
                ? statusCanSelectOnline(refreshedStatus)
                : false;
            setOnlineAfterOnboarding(onlineReady);
            if (
                onlineReady &&
                nextUsername &&
                validateUsername(nextUsername) === null &&
                stage === "name"
            ) {
                setStage("memory");
        ...
        try {
            ...
            if (patch.launch_auth_mode === "online") {
                try {
                    ...
                    console.warn(
                        "Could not seed profile skin after onboarding Microsoft sign-in.",
                        err,
                    );
                }
            }
    ...
    if (stage === "name") {
        const authSignedIn = Boolean(
            authStatus?.msa_authenticated || onlineAfterOnboarding,
        );
        const authUnavailable =
            authState === "unavailable" || !microsoftSignInAvailable;
        const authDisabled =
            saving ||
            microsoftLogin.busy ||
            authState === "loading" ||
        ...
        const authProfileName =
            authStatus?.minecraft_profile?.name ?? authStatus?.username ?? "";
        const authLabel =
            authSignedIn && authProfileName
        ...
        widget = (
            ...
            above keeps the input and validation state visually grouped. */}
                <div
                    class="cp-ob-feedback"
                    data-state={
                        showNameError ? "error" : nameValid ? "hint" : "idle"
                    }
                >
                    {showNameError ? (
                ...
                {microsoftLogin.message && (
                    <div
                        class="cp-ob-auth-message"
                        data-tone={microsoftLogin.message.tone}
                    >
        ...
        subline = (
            <p class="cp-ob-subline">
                Your system has about {totalGB} GB. {rec.rec} GB is a
        ...
        widget = (
            <div class="cp-ob-widget cp-ob-memory">
                <Slider
                    ...
                    step={0.5}
                    recommended={[
                        Math.max(2, rec.rec - 2),
                        Math.min(totalGB, rec.rec + 2),
                    ]}
                    sound="memory"
                    onChange={setMemory}
                ...
                <div class="cp-ob-memory-summary" aria-live="polite">
                    ...
                    <div class="cp-ob-memory-note">
                        Comfortable start: {rec.rec} GB
                    </div>
                </div>
            </div>
        );
        ...
        subline = (
            <p class="cp-ob-subline">
                Drag anywhere, tap a preset, or flip the canvas. Everything
                learns your color.
            </p>
        );
        widget = (
            <div class="cp-ob-widget">
                <div class="cp-ob-mode-row">
                    <AccentModeToggle
                        onChange={(m) => setIsWeirdo(m === "light")}
                    />
                    {isWeirdo && (
        ...
        subline = (
            <p class="cp-ob-subline">
                Ambient music pauses itself when the game starts.
            </p>
        );
        widget = (
            <div class="cp-ob-widget">
