...
Core Minecraft derives a bounded deterministic known-good artifact inventory as factual metadata, not as a Guardian decision or repair capability. Sealed producer inputs bind authenticated vanilla metadata, exact loader source proofs, a checksum-validated asset index, and a checksum-validated managed-runtime component manifest before derivation emits closed safe-relative launcher-owned entries. Every regular file has an exact SHA-1 and mandatory numeric size sourced only from authenticated retained bytes, authenticated declarations, or the same full-hash observation that established the digest. Invalid paths, missing or invalid checksums or sizes, identities, conflicts, impossible runtime ancestor trees, and bounds fail closed; public errors carry no provider paths. Runtime manifest proof bytes are canonical, and runtime executable and ready-marker semantics are explicit. Checksumless profile sources must cross a fresh bounded stream and have their observed hash and size authored into installed metadata before the receipt exists; neither a filesystem walk, State inventory, persisted evidence, nor current disk bytes can bless content. User-owned and unknown-owned roots such as mods, config, saves, and resource packs are absent by construction. State startup requires the exclusive known-good persistence owner and has no volatile fallback. It accepts fresh exact receipts before install success, reconciles persistence and admits any required exact snapshot revision before activating inventory under exact registered identity and root authority, and persists a strict rebuildable v3 per-instance snapshot. It rejects v2 and incomplete-integrity bytes without a compatibility reader, silently replaces missing, malformed, or stale bytes from an accepted fresh inventory, retries the exact failed atomic revision, and retires exact live and persisted lanes with the registered instance. Persisted snapshots are never loaded into runtime authority. There is still no launch-preflight verification tier or new Guardian repair authority; the inventory remains distinct from the private selected descriptors that authorize today's bounded artifact repair.
