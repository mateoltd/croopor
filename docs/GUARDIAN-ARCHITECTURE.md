...
Loader reconstruction extends that same effect-free Core primitive to Fabric and Quilt profiles, earliest pre-installer Forge, and declarative installer-era Forge/NeoForge shapes. Installer reconstruction authenticates the fresh fixed installer and sidecar, streams only incomplete library declarations, and may omit processor execution only when no client work exists or every terminal output has authenticated SHA-1 and positive size. Missing terminal sizes and outputless processor profiles remain unsupported until owned ephemeral execution exists. Reconstruction emits factual authority only: no repair descriptor, Guardian authorization, publication obligation, or mutation capability is created, and source acquisition or proof failure fails closed. Installed files, catalog caches, persisted known-good snapshots, and other local evidence cannot substitute for a missing or invalid fresh source. Application orchestration and State reconciliation/activation wiring remain pending; until those boundaries consume the move-only reconstruction result, Core reconstruction alone changes no live or durable authority.
