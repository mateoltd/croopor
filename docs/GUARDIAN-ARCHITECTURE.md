...
Core Minecraft derives a bounded deterministic known-good artifact inventory as factual metadata, not as a Guardian decision or repair capability. Sealed producer inputs bind authenticated vanilla metadata, exact loader source proofs, a checksum-validated asset index, and a checksum-validated managed-runtime component manifest before derivation emits closed safe-relative launcher-owned entries. Invalid paths, missing or invalid checksums, identities, conflicts, impossible runtime ancestor trees, and bounds fail closed; public errors carry no provider paths. Runtime manifest proof bytes are canonical, runtime executable and ready-marker semantics are explicit, and every library entry has an exact SHA-1 contract. Checksumless profile sources must cross a fresh bounded stream and have their observed hash and size authored into installed metadata before the receipt exists; neither a filesystem walk, State inventory, persisted evidence, nor current disk bytes can bless content through a structural exception. User-owned and unknown-owned roots such as mods, config, saves, and resource packs are absent by construction. State accepts fresh exact receipts before install success, activates their inventory only under exact registered identity and root authority, and persists a strict rebuildable v2 per-instance snapshot. It rejects v1 and structural-integrity bytes without a compatibility reader, silently replaces missing, malformed, or stale bytes from an accepted fresh inventory, retries the exact failed atomic revision, and retires exact live and persisted lanes with the registered instance. Persisted snapshots are never loaded into runtime authority. There is still no launch-preflight verification tier or new Guardian repair authority; the inventory remains distinct from the private selected descriptors that authorize today's bounded artifact repair.
