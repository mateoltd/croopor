...
- one identity-bound managed-composition runtime authority: registered instance ids resolve to registry-owned storage, per-instance inspection and mutation are serialized, accepted mutations survive caller cancellation, active sessions/launch/deletion exclude mutation through the shared instance lifecycle, indeterminate effects latch fail-closed, and shutdown drains the authority before closing the instance registry
...
For instance-scoped work, State requires a canonical registered instance id and derives its exact storage from the instance registry. It serializes inspection, health, evidence, rollback list/preflight, install, remove, and rollback per instance while permitting unrelated instances to progress. Reads use only this managed per-instance boundary and remain available while a game session runs. Mutation first rejects an active session, then takes the shared instance lifecycle boundary before managed admission, resolves the registry entry again, and rechecks the session before effects. Launch and instance deletion use that same lifecycle boundary, so they cannot overlap mutation, and queued or resumed work cannot recreate a deleted instance.

An accepted mutation moves to an App-owned task before its caller waits, so caller cancellation cannot abandon or overlap the effect. If the effect outcome is indeterminate, State latches that instance and rejects later inspection and mutation; shutdown also reports incomplete instead of releasing authority or closing the instance registry. This is a fail-closed boundary, not durable recovery: a later recovery implementation must establish the state before the latch can be cleared.
...
- Indeterminate managed composition effects block later access and clean shutdown instead of being reported as success.
