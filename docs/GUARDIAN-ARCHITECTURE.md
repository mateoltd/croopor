...
Core Minecraft currently derives a bounded deterministic known-good artifact inventory as factual metadata, not as a Guardian decision or capability. Sealed producer inputs bind the resolved effective version identity, typed loader build identity, fallible library plans, checksum-validated asset index, and checksum-validated managed-runtime component manifest before derivation emits closed safe-relative launcher-owned entries. Invalid paths, checksums, identities, conflicts, impossible runtime ancestor trees, and bounds fail closed; public errors carry no provider paths. Runtime manifest proof bytes are canonical, runtime executable and ready-marker semantics are explicit, and only exact typed loader-profile or extracted-installer paths can receive a structural unverified jar contract while base libraries remain strict. Neither filesystem walks nor downstream current disk bytes can bless content. User-owned and unknown-owned roots such as mods, config, saves, and resource packs are absent by construction. State owns a strict rebuildable per-instance snapshot, silently replaces missing, malformed, or stale bytes from an accepted fresh inventory, retries the exact failed atomic revision, and retires the exact cache lane with the registered instance. The sealed live-source facade is not wired yet, so production workflows do not materialize or consume these records. There is still no launch-preflight verification tier or new Guardian repair authority; the inventory remains distinct from the private selected descriptors that authorize today's bounded artifact repair.
