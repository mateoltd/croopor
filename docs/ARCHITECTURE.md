...
The instance overview reads `GET /api/v1/launch/preflight/{instance_id}` for a backend-authored Guardian preflight before launch. That endpoint validates the configured library and instance existence, reuses the same launch-preparation policy for Guardian mode, override origins, effective memory, clamp/low-memory warnings, resource pressure warnings, and Custom-mode risky override warnings, and returns only bounded JSON facts: Guardian summary, effective memory, override origins/booleans, and scalar resource pressure. It never creates a launch session, starts Minecraft, installs files, ensures instance layout, writes proof state, or exposes filesystem paths, command lines, raw JVM args, account names, usernames, or tokens.
...
Croopor currently launches with an offline identity from config. The active player name is validated through `core/config`, launch preparation builds an explicit internal launch-auth context from that offline identity, and no persistent Microsoft token storage is active yet. The launch planner consumes that typed auth context when substituting Minecraft launch variables, but runtime launch credentials remain offline-only until the real profile/token chain is deliberately wired into launch.

The API exposes `GET /api/v1/auth/status` as the current account boundary. Today it validates the configured username and always returns the launch identity as an offline identity: offline mode, deterministic offline UUID, unverified identity, online-mode not ready, default skin source, and login availability based on the optional public `CROOPOR_MSA_CLIENT_ID`. When a Microsoft device-code poll and the downstream Minecraft auth chain have completed in the current process, the same status response also reports bounded restart-volatile MSA sign-in state plus Minecraft profile and ownership readiness metadata: profile id/name, typed skin/cape metadata, Java ownership verification, and Minecraft token expiry seconds. These fields do not change launch credentials, and `online_mode_ready` remains false. `POST /api/v1/auth/login` is the login-start boundary. Without that client id it returns a JSON unavailable response; with the client id it requests a Microsoft device-code challenge using the `XboxLive.signin offline_access` scope, stores the raw Microsoft `device_code` in a restart-volatile in-memory `AuthLoginStore`, and returns a public pending response with a local `login_id`, user code, verification URL, expiry, interval, and optional message. `GET /api/v1/auth/login/{login_id}` reads that local session without contacting Microsoft: pending sessions return non-sensitive public metadata with bounded remaining seconds, expired sessions return `410 Gone`, and unknown sessions return `404`. `POST /api/v1/auth/login/{login_id}/poll` is the explicit one-shot Microsoft token polling boundary. Each request performs at most one bounded token-endpoint request with the stored server-side `device_code`, maps expected device-flow outcomes to public JSON, keeps `authorization_pending` and `slow_down` responses non-terminal without token fields, removes terminal declined/expired/bad-code sessions, and on Microsoft token success runs the internal Xbox Live/XSTS/Minecraft login/profile/ownership chain before storing active volatile account state. Full chain success replaces any previous active volatile MSA token and Minecraft account slot while returning `msa_authenticated` and Minecraft readiness metadata without raw tokens or device codes. Chain failure after Microsoft token success returns bounded JSON, removes the pending session, and does not leave a partially signed-in active profile. `POST /api/v1/auth/logout` clears the active volatile MSA token slot, active volatile Minecraft account slot, and any pending device-code sessions, and is harmless when no one is signed in. Accounts & skins starts the device-code request, renders the user code, verification URL, expiry, and copy affordances inline, polls through the backend-owned one-shot poll route at the backend-provided interval, refreshes status when volatile MSA sign-in becomes active, and exposes logout for that volatile state. The frontend never owns the raw Microsoft device code or token material and keeps the launch identity copy offline/unverified until online-mode launch credentials are deliberately wired.

The API crate has an internal tested auth-chain client boundary in `apps/api/src/auth_chain.rs`. Given an MSA access token, it exchanges through Xbox Live user authentication, XSTS authorization, Minecraft `login_with_xbox`, Minecraft profile fetch, and Minecraft ownership fetch using injectable endpoints. The boundary has lightweight typed results for Xbox user hash, XBL/XSTS tokens, Minecraft access token, profile skins/capes, and ownership, and maps failures into bounded categories/messages without exposing tokens, device codes, usernames, commands, or raw provider payloads. The login poll route uses this client only after Microsoft device-code token success; it keeps the resulting MSA and Minecraft access tokens in restart-volatile memory for future launch wiring, avoids retaining XBL/XSTS tokens after the exchange, does not persist tokens, does not refresh tokens, and is not wired into online-mode launch credentials yet.
