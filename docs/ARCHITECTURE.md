...
For the Family C qualification route, `ready` additionally requires each target proof to carry backend-authored Guardian decision evidence and resource-budget memory, CPU, install, and disk evidence; missing evidence is reported through per-target descriptor-only reason ids.
