...
`core/performance` owns the bundled managed-performance manifest, cached remote manifest authority, plan resolution, bundle health vocabulary, emergency-disable evaluation, local rules-cache status, composition-owned artifact installation/removal, and local rollback snapshots for the last tracked managed bundle state. Normal API and desktop startup create or read `<config_dir>/performance/rules-cache.json`. When `CROOPOR_PERFORMANCE_RULES_URL` is unset or blank, the launcher records the bundled built-in manifest and performs no remote work. When the variable is configured, startup still does not fetch the network; it loads a cached valid remote manifest from the rules cache if one exists and validates, otherwise it falls back to the bundled built-in manifest with bounded diagnostics. Remote manifests are untrusted until parsed as the current manifest schema and accepted by `validate_manifest`. Invalid remote data is rejected as a whole and never partially applied. Signature verification is not implemented yet because the current manifest model has no signature fields. The API exposes this through `/api/v1/performance/*`:
...
- `POST /api/v1/performance/rules/refresh` is the only remote refresh trigger in this slice. It requires `CROOPOR_PERFORMANCE_RULES_URL`; when unset it returns HTTP 400 JSON `{ "error": "performance remote rules url is not configured" }`. When configured, it performs a bounded-time, bounded-body fetch, parses a `Manifest`, validates it with `validate_manifest`, persists the accepted manifest as the active remote rules cache, swaps the in-memory active rules, and returns normal performance rules status. Fetch, parse, size, validation, or cache-write failures leave the previous active rules unchanged and expose a compact warning in status.
