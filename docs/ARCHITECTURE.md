...
- The frontend should prefer backend-authored Guardian outcomes; when Guardian has authored actionable `blocked`, `warned`, or `intervened` details, those details own the launch notice, and Healing details are only supporting diagnostics for incomplete or non-actionable Guardian payloads.
