...
Installed loader versions are classified by their declared Minecraft target, never by the opaque local version id. Current launcher-authored profiles carry an exact `inheritsFrom` even though their launch model is already materialized. `axialMaterialized = true` prevents a second inheritance merge only after the strict current sidecar proves the recomputed folder id, profile id, component, loader version, and declared parent agree. Missing, old, forged, or mismatched provenance fails closed. The validated declared parent then drives Minecraft display, lifecycle, Java inheritance, readiness, and ordering. Metadata cannot supply a missing parent as a compatibility fallback.
