...
Reconstruction is reachable publicly only by a strict canonical `loader-v2` installed-version id. Core derives the provider URL and strategy itself; no caller record, catalog, cache, path, or installed file supplies authority. Fabric and Quilt consume fresh fixed profile proof/profile sources and source-only vanilla reconstruction. Earliest Forge consumes a fresh SHA-1-sidecar-authenticated archive and fresh authenticated base client. Installer-era Forge and NeoForge consume a fresh fixed installer with its strict sidecar, bind the same typed declarations as installation, skip exact and embedded library requests, and stream incomplete declarations without preparing destinations. No-work installers reconstruct directly. Runnable processor plans reconstruct only when every terminal output already has exact SHA-1 and positive size; missing sizes and processors without authenticated terminal declarations fail before vanilla or library acquisition because ephemeral processor execution is not yet supported. These paths produce a distinct reconstruction receipt through the same private inventory derivation as install and perform no destination or publication effects.
