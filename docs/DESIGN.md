...
- The look guardian (`ui/look-guardian.ts`) prevents bad colors instead of warning about them: palettes are Poisson-disk samples (seed-decorrelated, min ~22° apart) over the theme-harmonious hue domain (25–80° analogous or 150–180° complementary offsets, minus the swamp rule: hues 95–165 only when analogous to the theme, so pink themes never get their green "complement"). Shuffle regenerates the whole palette and the selected color is always one of the visible swatches. Its warn-shaped verdict (`assessTileHue`) exists for surfaces where prevention is impossible, such as existing instances after a theme change.
