...
- Raised surfaces are lit from above: a 1px inset catch-light (`inset 0 1px 0 oklch(1 0 0 / 0.05)`, cards `0.07`) rides on top of `--shadow-raised`. Compact accent-fill controls (rail nav, tab rows, choice pills, primary buttons) wear the lens treatment: `inset 0 1px 0 oklch(1 0 0 / 0.14)` plus a `0 1px 0 oklch(0 0 0 / 0.18)` contact line. Depth never comes from heavier drop shadows; the deep `--shadow` is for floating overlays only.
...
- The look guardian (`ui/look-guardian.ts`) prevents bad colors instead of warning about them: palettes are seed-decorrelated, gap-relaxed samples over seeded profiles inside the theme-harmonious hue domain (25–80° analogous offsets only; no complementary jumps, because the instance art and aurora sit next to the live accent). Sampling is bounded and falls back to deterministic, evenly spaced profile-domain hues. Shuffle regenerates the whole palette and the selected color is always one of the visible swatches. Existing instance seeds resolve through the same guard at render time so theme changes do not leave clashing art behind.
...
- The tab row is section navigation and uses the selection language: ghost icon+label buttons, hover `--surface-2` with the rail's hover lift, active `--accent-fill`/`--accent-on` with the lens treatment. Labels collapse to icons below 740px; never a scrolling tab strip.
...
- Instance settings: one raised sheet of hairline-divided rows using the shared settings primitives (`ui/SettingsSheet.tsx`: `SettingsSection`/`SettingRow`/`OverrideChip`, `.cp-sheet*`), no master-detail nav, decoupled from the logs UI. Global settings use the same sheet language — there is no card-per-setting layout. Every control auto-saves on commit (no Save button); instance rows that can inherit the global default show a neutral "Overridden" pill with a quiet Reset that writes the empty/zero sentinel. Persistent mode choices (performance, guardian, launch profile) use `ui/ChoicePills.tsx`: a compact pill radiogroup with the accent-fill selection language, the selected option's note rendered as the row description, no icons. Backend performance-health warnings render as a `.cp-notice` above the sheet.
