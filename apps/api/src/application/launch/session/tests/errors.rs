...
#[test]
fn launch_instance_root_error_response_keeps_500_and_hides_unix_path_material() {
    let (status, Json(body)) = launch_instance_root_error_response(
...
#[test]
fn launch_instance_root_error_response_hides_windows_path_material_and_raw_io_text() {
    let (status, Json(body)) = launch_instance_root_error_response(
    ...
    assert_eq!(
        body["error"],
        "Could not access the registered instance folder. Check app data permissions and try again."
    );
    let body = body.to_string();
    for fragment in [
        r"C:\Users\Alice",
